...
export const QUERY_TEMPLATES: Record<string, QueryTemplate> = {
  ...
  DECISIONS_BY_LOCATION: {
    ...
    pattern: /החלטות\s+(?:על|לגבי)\s+([\u0590-\u05FF\s]+?)\s+(?:ש)?(?:התקבלו\s+)?(?:מאז|מ[-\s]?)(\d{4})/,
    ...
    priority: 3  // Much lower priority to avoid catching topic+year queries
  },
  ...
  // X decisions by topic and year - NEW
  X_DECISIONS_BY_TOPIC_AND_YEAR: {
    name: 'X החלטות בנושא ושנה',
    description: 'מספר מדויק של החלטות בנושא מסוים משנה מסוימת',
    pattern: /(?:הבא|תן|הצג)\s+(?:לי\s+)?(\d{1,2})\s+החלטות\s+בנושא\s+([\u0590-\u05FF\s]+?)\s+(?:מ|משנת)\s*(\d{4})/,
    sql: (count: string, _topic: string, _year: string) => `
      SELECT * FROM israeli_government_decisions 
      WHERE tags_policy_area ILIKE $1
      AND EXTRACT(YEAR FROM decision_date) = $2
      ORDER BY decision_date DESC
      LIMIT ${parseInt(count)}
    `,
    params: (_count: string, topic: string, year: string) => [`%${topic.trim()}%`, parseInt(year)],
    expectedType: 'multiple',
    examples: ['הבא 5 החלטות בנושא בריאות מ2024', 'תן לי 10 החלטות בנושא חינוך משנת 2023'],
    priority: 12
  },

  // Decisions by topic and year (no specific count)
  DECISIONS_BY_TOPIC_AND_YEAR: {
    name: 'החלטות בנושא ושנה',
    description: 'החלטות בנושא מסוים משנה מסוימת',
    pattern: /החלטות\s+בנושא\s+([\u0590-\u05FF\s]+?)\s+(?:מ|משנת)\s*(\d{4})/,
    ...
    expectedType: 'multiple',
    examples: ['החלטות בנושא בריאות מ2024', 'החלטות בנושא חינוך משנת 2023'],
    priority: 11
  },

  // X decisions by topic and year - COMPREHENSIVE PATTERN
  X_DECISIONS_BY_TOPIC_AND_YEAR_COMPREHENSIVE: {
    name: 'X החלטות בנושא ושנה (מקיף)',
    description: 'מספר מדויק של החלטות בנושא מסוים משנה מסוימת',
    pattern: /(?:הבא|תן|תציג|הצג)?\s*(?:לי\s+)?(?:(\d+)\s+החלטות|החלטה(?:\s+אחת)?)\s+בנושא\s+([\u0590-\u05FF\s]+?)\s+(?:מ|משנת)\s*(\d{4})/,
    sql: (countOrOne: string, topic: string, year: string) => {
      const limit = countOrOne === 'החלטה' || countOrOne === undefined ? 1 : parseInt(countOrOne);
      return `
        SELECT * FROM israeli_government_decisions 
        WHERE tags_policy_area ILIKE $1
        AND EXTRACT(YEAR FROM decision_date) = $2
    ...
    params: (_countOrOne: string, topic: string, year: string) => [`%${topic.trim()}%`, parseInt(year)],
    expectedType: 'multiple',  // Will be handled dynamically by formatter
    examples: [
      'החלטה אחת בנושא בריאות מ2024',
      '5 החלטות בנושא חינוך משנת 2023',
      'הבא 20 החלטות בנושא תחבורה מ2022',
      'תן לי 10 החלטות בנושא ביטחון משנת 2024'
    ],
    priority: 14  // Very high priority to catch before other patterns
  },

  // Single decision by topic with year (legacy support)
  SINGLE_DECISION_BY_TOPIC_AND_YEAR: {
    name: 'החלטה בודדת לפי נושא ושנה',
    description: 'מציאת החלטה אחת בנושא מסוים משנה מסוימת',
    pattern: /(?:הבא\s+)?(?:לי\s+)?החלטה\s+(?:אחת\s+)?בנושא\s+([\u0590-\u05FF\s]+?)\s+(?:מ|משנת)\s*(\d{4})/,
    sql: (_topic: string, _year: string) => `
      SELECT * FROM israeli_government_decisions 
      WHERE tags_policy_area ILIKE $1
      AND EXTRACT(YEAR FROM decision_date) = $2
    ...
    params: (topic: string, year: string) => [`%${topic.trim()}%`, parseInt(year)],
    expectedType: 'single',
    examples: ['החלטה אחת בנושא בריאות מ2024', 'הבא החלטה בנושא חינוך משנת 2023'],
    priority: 10
  },
