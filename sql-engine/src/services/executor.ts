  ...
  private async executeWithRPC(sql: string, params: any[]): Promise<any> {
    ...
    params.forEach((param, index) => {
      ...
      if (param === null) {
        ...
        // IMPORTANT: All other types (including numbers) should be treated as strings
        // because most columns in the database are TEXT type
    ...
    if (Array.isArray(data) && data.length > 0 && data[0].result) {
      ...
      return {
        success: true,
        data: resultData.length === 1 ? resultData[0] : resultData,
        row_count: resultData.length
      };
    }
    
    // For direct function calls that return data directly
    return {
      success: true,
      data: data,
